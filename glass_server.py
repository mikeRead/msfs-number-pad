...
#   'TURN COORDINATOR BALL' Turn coordinator ball position [-127 to 127]
#]
...
@app.route('/sixpack')
def sixpack():
	return render_template("dashboardViertical.html")
...
def output_ui_variables2():
	...
	try:
		ui_friendly_dictionary["AIRSPEED_INDICATE"] = (aq.get("AIRSPEED_INDICATED"))
		ui_friendly_dictionary["ALTITUDE"] = thousandify(round(aq.get("PLANE_ALTITUDE")))
		...
		ui_friendly_dictionary["TURN_COORDINATOR_BALL"] = aq.get("TURN_COORDINATOR_BALL")
		ui_friendly_dictionary["HEADING_INDICATOR"] = aq.get("HEADING_INDICATOR") 
		ui_friendly_dictionary["VERTICAL_SPEED"] = aq.get("VERTICAL_SPEED") 
		ui_friendly_dictionary["AUTOPILOT_HEADING_LOCK_DIR"] = aq.get("AUTOPILOT_HEADING_LOCK_DIR")
		ui_friendly_dictionary["KOHLSMAN_SETTING_HG"] = aq.get("KOHLSMAN_SETTING_HG")
		ui_friendly_dictionary["SIM_ON_GROUND"] = aq.get("SIM_ON_GROUND")
	except:
		print("err")
...
def trigger_event(event_name, value_to_use = None):
	...
	if EVENT_TO_TRIGGER is not None:
		if value_to_use is None:
		...
		elif event_name == "COM_RADIO_SET" or event_name == "COM2_RADIO_SET" or event_name == "COM_STBY_RADIO_SET" or event_name == "NAV1_STBY_SET" or event_name == "NAV1_RADIO_SET" or event_name == "XPNDR_SET":
			...
			if flag_3dec is True and str(value_to_use)[-2:] != "25" and str(value_to_use)[-2:] != "75":
				if event_name == "COM_RADIO_SET":
					...
					trigger_event("COM_STBY_RADIO_SWAP")
				elif event_name == "COM_STBY_RADIO_SET":
					trigger_event("COM_STBY_RADIO_SET")
